#!/sbin/sh
 ...
 # Dimodifikasi untuk menginstall AGK oleh ZmR
 #
...
cp /tmp/init.slim.rc /tmp/ramdisk/
chmod 0750 /tmp/ramdisk/init.slim.rc
if [ $(grep -c "import init.slim.rc" /tmp/ramdisk/init.qcom.rc) == 0 ]; then
   sed -i "/import \init\.qcom\.usb\.rc/aimport init.slim.rc" /tmp/ramdisk/init.qcom.rc
fi
